...
def encode(input_dir, output_filename, ext='.png'):
  ...
  fs = [os.path.join(input_dir, x) for x in np.sort(os.listdir(input_dir)) if ext in x]
  num_imgs = len(fs)
  with open(output_filename, "wb") as output_file:
      ...
      for i in range(num_imgs):
        print(i)
        with Image.open(fs[i]) as img:
            im = np.asarray(img.convert('L'), dtype=np.uint32)
...
if __name__ == '__main__':
    im_dir = os.path.join("images","a_decouper", "test", "imagettes_176x96")
    data_dir = os.path.join("images","encode", "imagettes_176x96")
    encode(im_dir, os.path.join(data_dir,"test"), ext='.png')
